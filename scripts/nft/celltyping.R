...
sce <- read_sce("~/SCADSplice/scrnaseq/nft/results/sce_merged", read_metadata = T)
...
write_sce(
  sce,
  file.path("~/SCADSplice/scrnaseq/nft/results/sce_integrated"),
  write_metadata = TRUE
)
...
png("~/SCADSplice/scrnaseq/nft/results/plots/umap_manifest.png",
...
dev.off()
png("~/SCADSplice/scrnaseq/nft/results/plots/umap_diagnosis.png",
    width = 170, height = 130, units = "mm", res = 600)
...
dev.off()
png("~/SCADSplice/scrnaseq/nft/results/plots/umap_age.png",
    width = 170, height = 130, units = "mm", res = 600)
plot_reduced_dim(sce, feature_dim = "age", reduced_dim = "UMAP_Liger",
  ...
  theme(legend.title=element_blank())
dev.off()
png("~/SCADSplice/scrnaseq/nft/results/plots/umap_sex.png",
    width = 170, height = 130, units = "mm", res = 600)
...
dev.off()
png("~/SCADSplice/scrnaseq/nft/results/plots/umap_PMI.png",
    width = 170, height = 130, units = "mm", res = 600)
plot_reduced_dim(sce, feature_dim = "PMI", reduced_dim = "UMAP_Liger",
  ...
  theme(legend.title=element_blank())
dev.off()
png("~/SCADSplice/scrnaseq/nft/results/plots/umap_RIN.png",
    width = 170, height = 130, units = "mm", res = 600)
...
write_sce(
  sce,
  "~/SCADSplice/scrnaseq/nft/results/sce_clustered"
)

sce <- annotate_integrated_sce(
  sce,
  categorical_covariates = c("manifest","diagnosis", "age", "sex", "PMI", "RIN"),
...
png("~/SCADSplice/scrnaseq/nft/results/plots/umap_clusters.png",
...
p <- plot_reduced_dim(sce, feature_dim = "cluster_celltype", reduced_dim = "UMAP_Liger",
                      ...
                      alpha = 1)
neuset <- c("#FF0000", "#FF9900", "#FFE13", "#059F2D", "#014FE8", "#9101A1", "#BFBFBF",
            "#613915", "#73D7EE", "#FFAFC7")
...
png("~/ss_plots/umap_celltype.png",
    width = 170, height = 130, units = "mm", res = 600)
p
dev.off()
...
sce <- annotate_celltype_metrics(
  ...
  facet_vars = c("manifest","diagnosis", "age", "sex", "PMI", "RIN"),
  metric_vars = c("pc_mito","pc_ribo","total_counts","total_features_by_counts")
)

report_celltype_metrics(sce)

write_celltype_mappings(sce, folder_path = "~/SCADSplice/scrnaseq/nft/metadata/")
...
png("~/SCADSplice/scrnaseq/nft/results/plots/umap_genes2.png",
...
celltype_mappings <- read_celltype_mappings("~/SCADSplice/scrnaseq/nft/metadata/celltype_mappings.tsv")
...
png("~/SCADSplice/scrnaseq/nft/results/plots/umap_celltype_modify.png",
    width = 170, height = 130, units = "mm", res = 600)
p
dev.off()
...
dm <- setNames(data.frame(matrix(ncol = 15, nrow = 65)), c(as.character(unique(dt$Group))))
...
png("~/SCADSplice/scrnaseq/nft/results/plots/heatmap_celltype.png",
...
dm <- setNames(data.frame(matrix(ncol = 25, nrow = 92)), c(as.character(unique(dt$Group))))
rownames(dm) <- unique(dt$Gene)
for (i in c(as.character(unique(dt$Group)))) {
  dm[i] <- dt[dt$Group == i,3]
}
...
png("~/SCADSplice/scrnaseq/nft/results/plots/heatmap_clusters2.png",
...
write.table(celanot, "~/SCADSplice/scrnaseq/nft/metadata/nft_cells_annotation.tsv",
