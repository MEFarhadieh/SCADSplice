### Following codes were used FASTQ files to identify ASE & DTU & APA in nge
# barcodes filtering

ls ~/SCADSplice/scrnaseq/nge/trimed_data/*_2.fastq.gz | awk -F'/' '{print$6}' | awk -F'_' '{print$1}' >um.ID.txt
cat ~/um.ID.txt | while read id
do
umi_tools extract --bc-pattern=CCCCCCCCCCCCCCCCNNNNNNNNNN \
 --stdin ~/SCADSplice/scrnaseq/nge/trimed_data/${id}_2.fastq.gz \
 --stdout ~/SCADSplice/scrnaseq/nge/trimed_data/${id}_extracted_2.fastq.gz \
 --read2-in ~/SCADSplice/scrnaseq/nge/trimed_data/${id}_3.fastq.gz \
 --read2-out=~/SCADSplice/scrnaseq/nge/trimed_data/${id}_extracted_3.fastq.gz \
 --whitelist=~/SCADSplice/scrnaseq/nge/results/metadata/nge_whitelist/${id}.txt
done

# alignment

ls ~/SCADSplice/scrnaseq/nge/trimed_data/*_extracted_3.fastq.gz | awk -F'/' '{print$6}' | awk -F'_' '{print$1}' >star.ID.txt
...
--readFilesIn ~/SCADSplice/scrnaseq/nge/trimed_data/${id}_extracted_3.fastq.gz \
--readFilesCommand zcat \
--twopassMode Basic \
--alignIntronMax 1000000 \
--outFileNamePrefix ~/SCADSplice/scrnaseq/nge/results/${id}/2 \
...
cat ~/star.ID.txt | while read id
do
python3 scripts/light_class_input.py --outpath ~/SCADSplice/scrnaseq/nge/results/${id}/ \
...
--bams ~/SCADSplice/scrnaseq/nge/results/${id}/2Aligned.out.bam \
--UMI_bar \
--stranded_library
done
...
cat ~/star.ID.txt | while read id
do
Rscript scripts/GLM_script_light.R ~/SCADSplice/scrnaseq/nge/results/${id}/ \
...
cat ~/star.ID.txt | while read id
do
Rscript scripts/consolidate_GLM_output_files.R ~/SCADSplice/scrnaseq/nge/results/${id}/ \
${id} \
~/SCADSplice/references/SICILIAN_human_hg38_Refs/exon_pickle_file/hg38_refseq_exon_bounds.pkl \
...
cat ~/star.ID.txt | while read id
do
python3 scripts/Process_CI_10x.py -d ~/SCADSplice/scrnaseq/nge/results/${id}/ \
-o ${id} \
...
cat ~/star.ID.txt | while read id
do
Rscript scripts/post_processing.R ~/SCADSplice/scrnaseq/nge/results/${id}/ ${id}  1
done
